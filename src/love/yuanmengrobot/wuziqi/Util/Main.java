...
public class Main {
    ...
    private String filePathStatic;
    private String outTradeNoSt;

    public String getOutTradeNoSt() {
        return outTradeNoSt;
    }

    public void setOutTradeNoSt(String outTradeNoSt) {
        this.outTradeNoSt = outTradeNoSt;
    }

    public String getFilePathStatic() {
        return filePathStatic;
    }

    public void setFilePathStatic(String filePathStatic) {
        this.filePathStatic = filePathStatic;
    }
...
//        System.out.println("*&*");
////         测试查询当面付2.0交易
//        String result= main.test_trade_query(main.getOutTradeNoSt());
//        System.out.println(result+"^^^^^^^^^&&&&&&&&&^^^^^^^");
////        main.test_trade_query(main.getFilePathStatic());
//        in.nextInt();
//        System.out.println("*&*");
//         result= main.test_trade_query("tradeprecreate16204853204014294786");
//        System.out.println(result+"^^^^^^^^^&&&&&&&&&^^^^^^^");
...
//    }
    ...
    // 测试当面付2.0查询订单
    public String test_trade_query(String outStr) {
        ...
        return result.getTradeStatus().toString();
    }
    ...
    // 测试当面付2.0生成支付二维码
    public String test_trade_precreate(String money) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
...
//                + (long) (Math.random() * 10000000L);
        ...
        String subject = "源梦科技扫码消费";//CHJY，订单信息

        // (必填) 订单总金额，单位为元，不能超过1亿元
        // 如果同时传入了【打折金额】,【不可打折金额】,【订单总金额】三者,则必须满足如下条件:【订单总金额】=【打折金额】+【不可打折金额】
        String totalAmount = money;//CHJY,支付金额数
        ...
        // 业务扩展参数，目前可添加由支付宝分配的系统商编号(通过setSysServiceProviderId方法)，详情请咨询支付宝技术支持
        ExtendParams extendParams = new ExtendParams();
        extendParams.setSysServiceProviderId("2021000117642602");
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                String filePath = "src/love/yuanmengrobot/wuziqi/images/qr.png";
                outTradeNoSt=response.getOutTradeNo();
                log.info("filePath:" + filePath);
                ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);//输出二维码图片
